...
select emp_id, emp_name, job_code, salary
from employee e
where round(salary, -4) = (   -- round( int형 컬럼명, -4) 1만 단위로 반올림한다, -5는 10만단위!, 그냥 round 는 소수를 정수로 반올림
select round(avg(salary), -4)   
from employee
where job_code = e.job_code  -- 자기 자신 비교, 이 문장 아예 사라지면 전체와 평균급여 비교이고, WHERE job_code 이렇게만 쓰면 비교할 게 없어져 문법 오류
);
